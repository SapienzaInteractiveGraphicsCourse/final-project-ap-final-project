export class CharacterFactory {
	...
	buildCharacter() {
		//Generate character
		this.headMesh = this.generateHeadMesh(0.6,0.6,0.6, 0, 0, 0);
		...
		this.bodyMesh = this.generateBodyMesh(0.6, 1.2, 0.45, 0, -0.9, 0);
	...
	changeRotation(rotX) {
		...
		//this.rightArm.rotation.x=rotX;
	}
	
	smashArmRight(){
		var temp=this.rightArm.rotation.z;		
			this.rightArm.rotation.z = temp+Math.PI/2;
			//console.log(this.rightArm.rotation.z );
	}
	...
	generateBoxMesh(width, height, depth, x, y, z) {
		...
		var boxMaterial = new THREE.MeshPhongMaterial( { color: 0x8D7575 } );
	...
	generateHeadMesh(width, height, depth, x, y, z) {
		...
		new THREE.MeshPhongMaterial({color: 0x8D7575}),
		new THREE.MeshPhongMaterial({color: 0x8D7575}),
		new THREE.MeshPhongMaterial({map: new THREE.TextureLoader().load('resources/images/face.png')})]
	...
	generateBodyMesh(width, height, depth, x, y, z) {
		var boxGeometry = new THREE.BoxGeometry(width, height, depth);
		var cubeMaterials=
		[new THREE.MeshPhongMaterial({color: 0x8D7575}),
		new THREE.MeshPhongMaterial({color: 0x8D7575}),
